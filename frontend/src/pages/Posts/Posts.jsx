...
const Posts = () => {
  ...
  const handleUpdatePost = async (postId, updatedData) => {
    try {
      const response = await axios.patch(`${process.env.REACT_APP_API_URL}/posts/${postId}`, updatedData, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const inkpositiveLikes = (postId, currentPositiveLikes, currentNegativeLikes) => {
  ...
  const inknegativeLikes = (postId, currentPositiveLikes, currentNegativeLikes) => {
                  ...
                  <Card sx={{minWidth: 275}}>
                    <CardContent>
                      ...
                      <Typography variant="h2" component="h1" gutterBottom>
                        <Link to={`/post/${post._id}`}>{indexOfFirstPost + index + 1}. {post.title}</Link>
                      </Typography>
                      ...
                      {post.viewsCount ? (
                        <Typography variant="body2" color="textSecondary" sx={{display: 'flex', alignItems: 'center'}}>
                          <VisibilityIcon sx={{marginRight: 0.5}}/> {post.viewsCount}
                        </Typography>
                      ) : (
                        <Typography variant="body2" color="textSecondary">
                        <span>
                               It has not been viewed yet.
                        </span>
                        </Typography>
                      )
                      }
                      {/*likes*/}
                      <Typography variant="body2" color="textSecondary" sx={{display: 'flex', gap: '5px'}}>
                        <ThumbUpIcon onClick={() => {
                          inkpositiveLikes(post._id, post.positiveLikes, post.negativeLikes)
                        }}
                        /> {post.positiveLikes}
                      </Typography>
                      <Typography variant="body2" color="textSecondary" sx={{display: 'flex', gap: '5px'}}>
                        <ThumbDownIcon onClick={() => {
                          inknegativeLikes(post._id, post.positiveLikes, post.negativeLikes)
                        }}/> {post.negativeLikes}
                      </Typography>
                      {/*EditPost*/} {/*Delete*/}
                      {isAuthenticated && currentUser && post.user && post.user._id === currentUser._id && (
